...
(function($wnd, $doc){
  ...
  var badBrowser = 'Unable to load Super Dev Mode of "riskanalysis" because\n';
  ...
  $wnd.__gwt_activeModules['riskanalysis'] = {
    'moduleName' : 'riskanalysis',
    'bindings' : function() {
      return {};
    }
  };
 ...
 * "/riskanalysis.nocache.js" is the one that loaded us. Normally this happens
...
function computeScriptBase() {
  ...
  $wnd.__gwt_activeModules['riskanalysis'].superdevmode = true;

  var expectedSuffix = '/riskanalysis.nocache.js';
  ...
  $wnd.alert('Unable to load Super Dev Mode version of ' + riskanalysis + ".");
}
;
  ...
  var devModeHookKey = '__gwtDevModeHook:riskanalysis';
  var devModeSessionKey = '__gwtDevModeSession:riskanalysis';

  // Compute some codeserver urls so as the user does not need bookmarklets
  var hostName = $wnd.location.hostname;
  var serverUrl = 'http://' + hostName + ':9876';
  var nocacheUrl = serverUrl + '/riskanalysis/riskanalysis.nocache.js';
  ...
  $wnd.__gwt_activeModules['riskanalysis'].canRedirect = true;
  $wnd.__gwt_activeModules['riskanalysis'].superdevmode = true;
  ...
  css = (moduleIdx == 1 ? css : '') +
    ".gwt-DevModeModule-" + moduleIdx + ".gwt-DevModeCompiling div:before{" +
      "content:'COMPILING riskanalysis';" +
      "font-size:24px;" +
      "color:#d2d9ee;" +
    "}";
  ...
  $wnd[compileFunction] = function() {
    ...
    compileScript.src = serverUrl +
      '/recompile/riskanalysis?user.agent=' + ua + '&_callback=' + callback;

...
(function($wnd, $doc){
  ...
  var badBrowser = 'Unable to load Super Dev Mode of "whatifanalysis" because\n';
  ...
  $wnd.__gwt_activeModules['whatifanalysis'] = {
    'moduleName' : 'whatifanalysis',
    'bindings' : function() {
      return {};
    }
  };
 ...
 * "/whatifanalysis.nocache.js" is the one that loaded us. Normally this happens
...
function computeScriptBase() {
  ...
  $wnd.__gwt_activeModules['whatifanalysis'].superdevmode = true;

  var expectedSuffix = '/whatifanalysis.nocache.js';
  ...
  $wnd.alert('Unable to load Super Dev Mode version of ' + whatifanalysis + ".");
}
;
  ...
  var devModeHookKey = '__gwtDevModeHook:whatifanalysis';
  var devModeSessionKey = '__gwtDevModeSession:whatifanalysis';

  // Compute some codeserver urls so as the user does not need bookmarklets
  var hostName = $wnd.location.hostname;
  var serverUrl = 'http://' + hostName + ':9876';
  var nocacheUrl = serverUrl + '/whatifanalysis/whatifanalysis.nocache.js';
  ...
  $wnd.__gwt_activeModules['whatifanalysis'].canRedirect = true;
  $wnd.__gwt_activeModules['whatifanalysis'].superdevmode = true;
  ...
  css = (moduleIdx == 1 ? css : '') +
    ".gwt-DevModeModule-" + moduleIdx + ".gwt-DevModeCompiling div:before{" +
      "content:'COMPILING whatifanalysis';" +
      "font-size:24px;" +
      "color:#d2d9ee;" +
    "}";
  ...
  $wnd[compileFunction] = function() {
    ...
    compileScript.src = serverUrl +
      '/recompile/whatifanalysis?user.agent=' + ua + '&_callback=' + callback;

...
(function($wnd, $doc){
  ...
  var badBrowser = 'Unable to load Super Dev Mode of "ras" because\n';
 ...
 * "/ras.nocache.js" is the one that loaded us. Normally this happens
...
function computeScriptBase() {
  ...
  $wnd.__gwt_activeModules['ras'].superdevmode = true;

  var expectedSuffix = '/ras.nocache.js';
  ...
  $wnd.alert('Unable to load Super Dev Mode version of ' + ras + ".");
}
;
  ...
  var devModeHookKey = '__gwtDevModeHook:ras';
  var devModeSessionKey = '__gwtDevModeSession:ras';

  // Compute some codeserver urls so as the user does not need bookmarklets
  var hostName = $wnd.location.hostname;
  var serverUrl = 'http://' + hostName + ':9876';
  var nocacheUrl = serverUrl + '/ras/ras.nocache.js';
  ...
  $wnd.__gwt_activeModules['ras'].canRedirect = true;
  $wnd.__gwt_activeModules['ras'].superdevmode = true;
  ...
  $wnd[compileFunction] = function() {
    ...
    compileScript.src = serverUrl +
      '/recompile/ras?user.agent=' + ua + '&_callback=' + callback;

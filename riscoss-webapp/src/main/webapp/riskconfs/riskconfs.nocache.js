...
(function($wnd, $doc){
  ...
  var badBrowser = 'Unable to load Super Dev Mode of "riskconfs" because\n';
  ...
  $wnd.__gwt_activeModules['riskconfs'] = {
    'moduleName' : 'riskconfs',
    'bindings' : function() {
      return {};
    }
  };
 ...
 * "/riskconfs.nocache.js" is the one that loaded us. Normally this happens
...
function computeScriptBase() {
  ...
  $wnd.__gwt_activeModules['riskconfs'].superdevmode = true;

  var expectedSuffix = '/riskconfs.nocache.js';
  ...
  $wnd.alert('Unable to load Super Dev Mode version of ' + riskconfs + ".");
}
;
  ...
  var devModeHookKey = '__gwtDevModeHook:riskconfs';
  var devModeSessionKey = '__gwtDevModeSession:riskconfs';

  // Compute some codeserver urls so as the user does not need bookmarklets
  var hostName = $wnd.location.hostname;
  var serverUrl = 'http://' + hostName + ':9876';
  var nocacheUrl = serverUrl + '/riskconfs/riskconfs.nocache.js';
  ...
  $wnd.__gwt_activeModules['riskconfs'].canRedirect = true;
  $wnd.__gwt_activeModules['riskconfs'].superdevmode = true;
  ...
  $wnd[compileFunction] = function() {
    ...
    compileScript.src = serverUrl +
      '/recompile/riskconfs?user.agent=' + ua + '&_callback=' + callback;

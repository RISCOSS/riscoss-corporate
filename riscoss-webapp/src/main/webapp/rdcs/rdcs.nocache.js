...
(function($wnd, $doc){
  ...
  var badBrowser = 'Unable to load Super Dev Mode of "rdcs" because\n';
 ...
 * "/rdcs.nocache.js" is the one that loaded us. Normally this happens
...
function computeScriptBase() {
  ...
  $wnd.__gwt_activeModules['rdcs'].superdevmode = true;

  var expectedSuffix = '/rdcs.nocache.js';
  ...
  $wnd.alert('Unable to load Super Dev Mode version of ' + rdcs + ".");
}
;
  ...
  var devModeHookKey = '__gwtDevModeHook:rdcs';
  var devModeSessionKey = '__gwtDevModeSession:rdcs';

  // Compute some codeserver urls so as the user does not need bookmarklets
  var hostName = $wnd.location.hostname;
  var serverUrl = 'http://' + hostName + ':9876';
  var nocacheUrl = serverUrl + '/rdcs/rdcs.nocache.js';
  ...
  $wnd.__gwt_activeModules['rdcs'].canRedirect = true;
  $wnd.__gwt_activeModules['rdcs'].superdevmode = true;
  ...
  $wnd[compileFunction] = function() {
    ...
    compileScript.src = serverUrl +
      '/recompile/rdcs?user.agent=' + ua + '&_callback=' + callback;

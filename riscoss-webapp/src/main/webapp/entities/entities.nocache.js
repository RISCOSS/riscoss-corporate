...
(function($wnd, $doc){
  ...
  var badBrowser = 'Unable to load Super Dev Mode of "entities" because\n';
  ...
  $wnd.__gwt_activeModules['entities'] = {
    'moduleName' : 'entities',
    'bindings' : function() {
      return {};
    }
 ...
 * "/entities.nocache.js" is the one that loaded us. Normally this happens
...
function computeScriptBase() {
  ...
  $wnd.__gwt_activeModules['entities'].superdevmode = true;

  var expectedSuffix = '/entities.nocache.js';
  ...
  $wnd.alert('Unable to load Super Dev Mode version of ' + entities + ".");
}
;
  ...
  var devModeHookKey = '__gwtDevModeHook:entities';
  var devModeSessionKey = '__gwtDevModeSession:entities';

  // Compute some codeserver urls so as the user does not need bookmarklets
  var hostName = $wnd.location.hostname;
  var serverUrl = 'http://' + hostName + ':9876';
  var nocacheUrl = serverUrl + '/entities/entities.nocache.js';
  ...
  $wnd.__gwt_activeModules['entities'].canRedirect = true;
  $wnd.__gwt_activeModules['entities'].superdevmode = true;
  ...
  $wnd[compileFunction] = function() {
    ...
    compileScript.src = serverUrl +
      '/recompile/entities?user.agent=' + ua + '&_callback=' + callback;

...
(function($wnd, $doc){
  ...
  var badBrowser = 'Unable to load Super Dev Mode of "layers" because\n';
  ...
  $wnd.__gwt_activeModules['layers'] = {
    'moduleName' : 'layers',
    'bindings' : function() {
      return {};
    }
 ...
 * "/layers.nocache.js" is the one that loaded us. Normally this happens
...
function computeScriptBase() {
  ...
  $wnd.__gwt_activeModules['layers'].superdevmode = true;

  var expectedSuffix = '/layers.nocache.js';
  ...
  $wnd.alert('Unable to load Super Dev Mode version of ' + layers + ".");
}
;
  ...
  var devModeHookKey = '__gwtDevModeHook:layers';
  var devModeSessionKey = '__gwtDevModeSession:layers';

  // Compute some codeserver urls so as the user does not need bookmarklets
  var hostName = $wnd.location.hostname;
  var serverUrl = 'http://' + hostName + ':9876';
  var nocacheUrl = serverUrl + '/layers/layers.nocache.js';
  ...
  $wnd.__gwt_activeModules['layers'].canRedirect = true;
  $wnd.__gwt_activeModules['layers'].superdevmode = true;
  ...
  $wnd[compileFunction] = function() {
    ...
    compileScript.src = serverUrl +
      '/recompile/layers?user.agent=' + ua + '&_callback=' + callback;
